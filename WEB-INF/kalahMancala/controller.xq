...
import module namespace csc = "kalahMancala/checkSpecialCases" at "checkSpecialCases.xq";
import module namespace cgo = "kalahMancala/checkGameOver" at "checkGameOver.xq";
...
(: Create a new game instance and refresh the database. :)
declare %rest:path("/gxf/newGame") %rest:GET updating function c:newGame() {
  ...
  return (insert nodes $newGame as first into $collection, db:output(cm:redirectToTransformator($gameId)), cm:refreshDatabase())
};
...
declare updating %rest:path('/gxf/specialCases/{$gameId}/{$startingPit}/{$numOfStepsToMove}/{$player}') %rest:GET function c:checkSpecialCases($gameId as xs:string, $startingPit as xs:string, $numOfStepsToMove as xs:decimal, $player as xs:decimal) {
  csc:checkSpecialCases($gameId, $startingPit, $numOfStepsToMove, $player)
};
...
declare updating %rest:path('/gxf/checkGameOver/{$gameId}') %rest:GET function c:checkGameOver($gameId as xs:string) {
  cgo:checkGameOver($gameId), db:output(cm:redirectToTransformator($gameId))
};
