...
declare updating %rest:path('/gxf/move/{$gameId}/{$house}') %rest:GET function ms:moveSeeds($gameId as xs:string, $house as xs:string) {
    ...
    return (replace value of node $pathToSeeds with $seedsToAdd,
       ms:distribute($seedsToDistribute, ms:getNumberFromTag($house), ms:getCurrentPlayer($gameId), $seedsToAdd - 1, $gameId))
};
